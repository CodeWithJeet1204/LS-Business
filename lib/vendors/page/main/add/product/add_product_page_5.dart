...
import 'package:ls_business/vendors/page/main/add/product/add_product_page_6.dart';
    ...
    if (productKey.currentState!.validate()) {
      if (property0.isEmpty && getCompulsory(0)) {
        if (getNoOfAnswers(0) == 1) {
          if (property0Controller.text.toString().trim().isEmpty) {
            return mySnackBar(
              context,
              'Enter value for ${getPropertiesKeys(0)}',
            );
          }
          property0
          ...
          return mySnackBar(
            context,
            'Add atleast one value to ${getPropertiesKeys(0)}',
          );
        }
      }
      if (property1.isEmpty && getCompulsory(1)) {
        if (getNoOfAnswers(1) == 1) {
          ...
          return mySnackBar(
            context,
            'Add atleast one value to ${getPropertiesKeys(1)}',
          );
        }
      }
      if (property2.isEmpty && getCompulsory(2)) {
        if (getNoOfAnswers(2) == 1) {
          if (property2Controller.text.toString().trim().isEmpty) {
            return mySnackBar(
              context,
              'Enter value for ${getPropertiesKeys(2)}',
            );
          }
          property2
      ...
      try {
        ...
        if (mounted) {
          Navigator.of(context).push(
            MaterialPageRoute(
              builder: (context) => const AddProductPage6(),
            ),
          );
        }
      } catch (e) {
